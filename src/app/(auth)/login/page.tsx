...
function LoginForm() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold font-[var(--font-heading)] mb-2 text-center">تسجيل الدخول</h1>
      <p className="text-sm text-text-muted text-center mb-8">ادخل إلى حسابك لتصفح وحفظ أعمالك المفضلة</p>

      <form onSubmit={handleSubmit} className="bg-surface rounded-3xl border border-border/50 p-8 shadow-xl space-y-4">
        {resetSuccess && <p className="text-green-500 text-xs text-center bg-green-500/10 p-3 rounded-xl">تم تغيير كلمة المرور بنجاح. يُرجى تسجيل الدخول.</p>}
        ...
        <div className="flex items-center justify-between">
          <button
            type="submit"
            disabled={loading}
            className="py-3.5 px-6 rounded-xl bg-accent text-white font-medium hover:bg-accent-dark transition-all shadow-lg shadow-accent/20 disabled:opacity-50 flex items-center justify-center gap-2"
          >
            {loading ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <><CheckIcon size={16} /> دخول</>}
          </button>
      ...
      <p className="text-center text-sm text-text-muted mt-6">
        ليس لديك حساب؟{" "}
        <Link href="/register" className="text-accent hover:text-accent-dark font-medium">إنشاء حساب</Link>
      </p>
    </div>
  );
}
